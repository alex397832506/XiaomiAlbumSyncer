...
- **飞牛**：套件内部是一个 Docker 容器，直接复用官方镜像，因此设备需要有容器运行时（fnOS 自带）。安装向导里可填写端口，装后也能在应用详情的设置页调整。
- **群晖 / 威联通**：套件自带精简 JRE 与 ExifTool，不依赖 NAS 上的 Java、Perl 或 Docker 环境。
  - 群晖安装时会出现设置页，可指定 Web 端口与数据目录；DSM 7 不允许未签名套件以 root 运行，所以套件以 `package` 用户身份启动并监听非特权端口（默认 8080）。
  - 威联通不支持安装时输入，沿用默认端口与数据目录，装后通过 `xas.env` 调整。另外，无签名应用在固件升级后可能不会自动启动，需要手动启动一次。
...
| 平台 | 安装向导 | 装后修改 |
| --- | --- | --- |
| 群晖 | ✅ 安装时会出现设置页，可填 **Web 端口** 与 **数据目录** | 修改数据目录下的 `xas.env`，重启套件 |
| 飞牛 | ✅ 安装时可填 **Web 端口**；装后在应用详情的**设置页**也能改 | 应用详情 → 设置页改端口，重启应用 |
| 威联通 | ❌ 该平台不支持安装时输入，沿用默认端口 8080 与套件数据目录 | 修改数据目录下的 `xas.env`，重启套件 |

群晖的数据目录默认在 `/var/packages/xiaomi-album-syncer/var/data`，填了自定义路径后
会自动软链接过去；威联通的目录在套件安装目录下的 `data/`。

#### 更多配置（`xas.env`）

除了端口，还可以在数据目录下放一个 `xas.env` 覆盖其它默认值：
